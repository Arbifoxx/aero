...
.DESCRIPTION
  Uses `ci/lib/Toolchain.psm1` to locate (and if needed install via winget or Microsoft's
  official standalone installers) MSBuild and Windows Kits tools used by downstream
  steps (Inf2Cat.exe, signtool.exe, stampinf.exe).
...
# Note: winget has its own internal cache. The standalone Microsoft bootstrappers are
# stored here so GitHub Actions can reuse them between workflow runs.
...
$kitToolchain = Ensure-WindowsKitToolchain -PreferredWdkWingetId 'Microsoft.WindowsWDK' -PreferredWdkKitVersion '10.0.19041.0'
...
$driverToolset = $kitToolchain.WindowsKits.DriverBuild.DriverPlatformToolset
$driverToolsetPlatforms = @($driverToolset.Platforms | ForEach-Object { $_.Platform }) -join ', '
Write-ToolchainLog -Message "WDK Visual Studio integration: $($driverToolset.Name) ($driverToolsetPlatforms)"
